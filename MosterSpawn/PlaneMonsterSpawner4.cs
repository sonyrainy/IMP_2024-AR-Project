...
public class PlaneMonsterSpawner4 : MonoBehaviour   // 스테이지 1 몬스터 스폰 스크립트
{
    ...
    void Update()
    {
        ...
        {
            Debug.Log("Stage 4 Clear!!");  //  스테이지 1 클리어!!
            phase += 1;
    ...
    void create1() 
    {
        ...
        for (int i = 0; i < 5; i++)     // 1페이즈는 몬스터 3마리 스폰되는데
        {
            ...
            GameObject monster = Instantiate(createRandom, spawnPosition, hitPose.rotation);  // prefab1P 몬스터 프리펩이 랜덤위치에 스폰
    ...
    void create2()
    {
        ...
        for (int i = 0; i < 10; i++)     // 2페이즈는 몬스터 6마리
        {
            int type = Random.Range(0, 4);
            switch (type)
            {
            ...
            Vector3 spawnPosition = ranpos + yoffset + hitPose.position;
            GameObject monster = Instantiate(createRandom, spawnPosition, hitPose.rotation); // prefab2P 몬스터 프리펩이 랜덤위치에 스폰
            Rigidbody rb = monster.GetComponent<Rigidbody>();
            spawnedObjects.Add(monster);
            monsterCount++;
        }
    ...
    void create3()
    {
        ...
        for (int i = 0; i < 15; i++)      // 3페이즈는 몬스터 9마리
        {
            int type = Random.Range(0, 4);
            switch (type)
            {
            ...
            Vector3 spawnPosition = ranpos + yoffset + hitPose.position;
            GameObject monster = Instantiate(createRandom, spawnPosition, hitPose.rotation);    // prefab3P 몬스터 프리펩이 랜덤위치에 스폰
            Rigidbody rb = monster.GetComponent<Rigidbody>();
            spawnedObjects.Add(monster);
            monsterCount++; 
        }
